...
class View {
      ...
      if($fnList){
		...
		function addStyle($style, $after = '', $source = true, $ts = false){
			if($source){
				return '<link rel="stylesheet" type="text/css" href="' . ADMROOT.STYLE . $style .'.css'. ( ((defined('DEVMODE') && DEVMODE === true) || $ts)?'?t='.uniqid():'' ) .'" '.$after.'/>'."\n\r";
			}else{
				return '<link rel="stylesheet" type="text/css" href="' . SSTYLE . $style .'.css'. ( ((defined('DEVMODE') && DEVMODE === true) || $ts)?'?t='.uniqid():'' ) .'" '.$after.'/>'."\n\r";
			}
		}
